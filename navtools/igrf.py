...
The older model is in the :file:`igrf11coeffs.txt` file.

The current model is in the :file:`igrf13coeffs.txt` file.

This can be downloaded from http://www.ngdc.noaa.gov/IAGA/vmod/igrf13coeffs.txt
...
provides usable, known unit test results for the IGRF-11 model.

Specifically, the file :file:`geomag70_linux/sample_out_IGRF13.txt`
provides usable, known unit test results for the IGRF-13 model.
...
::
    ...
    igrfsyn = igrf.IGRF("igrf11coeffs.txt")

    today = datetime.date.today()
    first_of_year = today.replace(month=1, day=1)
    date = today.year + (today.toordinal() - first_of_year.toordinal())/365.242

    x, y, z, f = igrfsyn(date, nlat*math.pi/180, elong*math.pi/180)
...
class IGRF:
    ...
    The IGRF calculation requires a complex table of coefficients
    loaded from :file:`./igrf11coeffs.txt`

    We defer coefficient loading until a call is made.
    ...
    def __init__(self, model: str = "./igrf11coeffs.txt") -> None:
        """Names the model coefficient file.

        Coefficients are loaded lazily when the first request is made.
        This avoids warning messages when testing or creating documentation.

        :param model: the coefficients file.
        """
        self.name = pathlib.Path(model)
        self.g: dict[int, dict[tuple[int, int], float]] = {}
        self.h: dict[int, dict[tuple[int, int], float]] = {}
        self.extrapolate: int = 0

    def prepare(self) -> None:
        """
        Locate the coefficients file and load them.
        This is a lazy loader, checked for each computation.
    ...
    def load_coeffs(
        ...
        """
        Returns two dictionaries of g and h coefficients by year and [n,m] index, and the year
        for which extrapolation must start.
        ...
        ..  todo:: Use urllib2
            ...
            Even though most use cases involve disconnected computers,
            and rely on a statically downloaded file. It's easy to cover
            *all* the bases by opening a URL instead of assuming a local file.
        ...
        with file_path.open() as source:
            ...
            # g/h n m -- a bunch of interpolation years -- and 2010-15 or 2020-25 for the extrapolation range.
            extrap_s, extrap_e = (
                match.groups()
                if (match := re.match(r"(\d\d\d\d)-(\d\d)", headings[-1]))
                else ("2010", "15")
            )
            years = [int(t[:-2]) for t in headings[3:-1]] + [
                int(extrap_s[:2] + extrap_e)
            ]
            for y in years:
            ...
            for i, row in enumerate(r.strip().split() for r in line_iter):
                ...
                for yr, coef in zip(years, coefs):
                    if g_or_h == "g":
                        ...
                        raise ValueError("Bad IGRF Coefficients")
    ...
    def __call__(
        ...
        :returns: tuple of x, y, z and f;
            ...
            The arctan of y/x is the final variance.
        """
        self.prepare()

        # Note: these arrays will use Fortran-style one-based indexing.
        p: dict[int, float] = {}
        q: dict[int, float] = {}
        cl: dict[int, float] = {}
        sl: dict[int, float] = {}
        ...
        if date < self.extrapolate:
            ...
            # Extrapolating past the given data.
            t = date - self.extrapolate
            tc = 1.0
            nmx = 13  # degrees
            kmx = (nmx + 1) * (nmx + 2) // 2  # total number of coefficients
...
def declination(
    ...
    print(
        f"igrfsyn({astro_dt_tm=}, {math.radians(nlat)=}, {math.radians(elong)=}, alt=0.0, coord='D'"
    )
    x, y, z, f = igrfsyn(
