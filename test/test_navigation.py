...
from navtools.navigation import (
    Angle, AngleParser,
    Lat, Lon, LatLon, KM, NM, declination, destination, range_bearing)
...
def test_range_bearing_2(case_2):
    ...
    d_nm, brg = range_bearing( p1, p2, R=NM )
    assert d_nm == approx(distance, rel=.1)
    assert math.degrees(brg) == approx(bearing, rel=.01)

def test_destination_2(case_2):
    p1, p2_expected, distance, bearing = case_2
    p2 = destination( p1, distance, bearing, R=NM )
    assert p2.lat == approx(p2_expected.lat, rel=.1) and p2.lon == approx(p2_expected.lon, rel=.1)
...
def test_declination_2(mock_datetime):
    ...
    assert d == approx(math.radians(-(5+26/60)), rel=.1/60)
